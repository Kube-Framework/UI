...
//     // Gradients
//     const vec2 pos = get_distance_vector(fragLeft - gl_FragCoord.xy, fragControl - gl_FragCoord.xy, fragRight - gl_FragCoord.xy);
// //     const vec2 dist = get_distance_vector(b0-p, b1-p, b2-p);
//     vec2 px = dFdx(pos);
//     vec2 py = dFdy(pos);
//     // Chain rule
//     float fx = (2 * pos.x) * px.x - px.y;
//     float fy = (2 * pos.x) * py.x - py.y;
//     // Signed distance
//     float d = (pos.x * pos.x - pos.y) / sqrt(fx * fx + fy * fy);
//     // Linear alpha
